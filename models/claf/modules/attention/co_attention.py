...
from .functional import add_masked_value
    ...
    * Args:
    ...
    def forward(self, context_embed, question_embed, context_mask=None, question_mask=None):
        ...
        if question_mask is not None:
            S_question = add_masked_value(S_question, question_mask.unsqueeze(1), value=-1e7)
        S_q = F.softmax(S_question, 2)  # (B, C_L, Q_L)

        S_context = S.transpose(1, 2)
        if context_mask is not None:
            S_context = add_masked_value(S_context, context_mask.unsqueeze(1), value=-1e7)
